...
def ets_with_diagostics(train, exog_train, exog_test, test, trend, seasonal, period, ljung_lags):
    seasonal_periods = period if seasonal is not None else None
    try:
        ets = ExponentialSmoothing(train, trend = trend, seasonal = seasonal, seasonal_periods = seasonal_periods).fit(optimized = True)
...
def search_ets(unique_id, train, exog_train, test, exog_test, trend_options, seasonal_options, period_options, ljung_lags):
    best = None
    results = []
        ...
        for seasonal in seasonal_options:
            for period in period_options:
                ...
                with warnings.catch_warnings():
                    ...
                    model = ets_with_diagostics(train, exog_train, exog_test, test, trend, seasonal, period, ljung_lags)
                if model is None:
                    continue
