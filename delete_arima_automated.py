...
def fit_sarima_with_diagnostics(train, exog_train, test, exog_test, order, seasonal_order, trend, ljung_lags):
    try:
        if seasonal_order[-1] == 0 and np.sum(seasonal_order[:-1]) != 0:
            return None
        model = SARIMAX(train, order = order, seasonal_order = seasonal_order, trend = trend, exog = exog_train, enforce_stationarity = False, enforce_invertibility = False).fit(disp = False)
    ...
    ljung_10 = acorr_ljungbox(model.resid, lags = ljung_lags[0], return_df = True)
    ljung_20 = acorr_ljungbox(model.resid, lags = ljung_lags[-1], return_df = True)
    ljung_pvalue = min(ljung_10['lb_pvalue'].iloc[-1], ljung_20['lb_pvalue'].iloc[-1])

    return {'hyperparameter_1': order, 'hyperparameter_2': seasonal_order, 'hyperparameter_3': trend, 'AIC': model.aic, 'BIC': model.bic, 'MAE': metrics['MAE'], 
            'RMSE': metrics['RMSE'], 'MAPE%': metrics['MAPE%'], 'MASE': metrics['MASE'], 'ljung_pvalue': ljung_pvalue, 'forecast': forecast, 'model': model}

def search_sarima_orders(train, exog_train, test, exog_test, p_values, d_values, q_values, s_values, trend_options, ljung_lags):
    best = None
    results = []
                ...
                for q in q_values:
                    for s in s_values:
                        if p == 0 and d == 0 and q == 0:
                            continue
                        res = fit_sarima_with_diagnostics(train, exog_train, test, exog_test, [p, d, q], [p, d, q, s], trend, ljung_lags)
                        if res is None:
                            continue
